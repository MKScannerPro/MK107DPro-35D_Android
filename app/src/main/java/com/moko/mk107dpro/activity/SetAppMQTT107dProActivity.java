...
import com.moko.mk107dpro.databinding.ActivityMqttApp107dproBinding;
import com.moko.mk107dpro.dialog.AlertMessageDialog;
import com.moko.mk107dpro.entity.MQTTConfig;
import com.moko.mk107dpro.fragment.General107dProFragment;
import com.moko.mk107dpro.fragment.SSL107dProFragment;
import com.moko.mk107dpro.fragment.User107dProFragment;
...
public class SetAppMQTT107dProActivity extends BaseActivity<ActivityMqttApp107dproBinding> implements RadioGroup.OnCheckedChangeListener {
    private final String FILTER_ASCII = "[ -~]*";
    private General107dProFragment generalFragment;
    private User107dProFragment userFragment;
    private SSL107dProFragment sslFragment;
    ...
    protected void onCreate() {
        ...
        expertFilePath = Main107dProActivity.PATH_LOGCAT + File.separator + "export" + File.separator + "Settings for APP.xlsx";
    }

    @Override
    protected ActivityMqttApp107dproBinding getViewBinding() {
        return ActivityMqttApp107dproBinding.inflate(getLayoutInflater());
    }

    private void createFragment() {
        fragments = new ArrayList<>();
        generalFragment = General107dProFragment.newInstance();
        userFragment = User107dProFragment.newInstance();
        sslFragment = SSL107dProFragment.newInstance();
    ...
    public void onMQTTConnectionCompleteEvent(MQTTConnectionCompleteEvent event) {
        ...
        ToastUtils.showToast(SetAppMQTT107dProActivity.this, getString(R.string.success));
...
//                    row10.createCell(1).setCellValue("");
                ...
                runOnUiThread(() -> {
                    ...
                    if (isFileError) {
                        isFileError = false;
                        ToastUtils.showToast(SetAppMQTT107dProActivity.this, "Export error!");
                        return;
                    }
                    ToastUtils.showToast(SetAppMQTT107dProActivity.this, "Export success!");
                    Utils.sendEmail(SetAppMQTT107dProActivity.this, "", "", "Settings for APP", "Choose Email Client", expertFile);
                    ...
                    new Thread(() -> {
                        try {
                            ...
                            if (rows < 13 || columns < 3) {
                                runOnUiThread(() -> {
                                    dismissLoadingProgressDialog();
                                    ToastUtils.showToast(SetAppMQTT107dProActivity.this, "Please select the correct file!");
                                });
                                return;
                            }
                            ...
                            runOnUiThread(() -> {
                                dismissLoadingProgressDialog();
                                if (isFileError) {
                                    ToastUtils.showToast(SetAppMQTT107dProActivity.this, "Import failed!");
                                    return;
                                }
                                ToastUtils.showToast(SetAppMQTT107dProActivity.this, "Import success!");
                                initData();
                            });
                        } catch (Exception e) {
                            e.printStackTrace();
