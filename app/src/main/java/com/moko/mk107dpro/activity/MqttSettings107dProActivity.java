...
public class MqttSettings107dProActivity extends BaseActivity<ActivityMqttDevice107dproBinding> implements RadioGroup.OnCheckedChangeListener {
    ...
    private SSLDevice107dProFragment sslFragment;
    ...
    @Override
    protected ActivityMqttDevice107dproBinding getViewBinding() {
        return ActivityMqttDevice107dproBinding.inflate(getLayoutInflater());
    }
    ...
    private void createFragment() {
        ...
        sslFragment = SSLDevice107dProFragment.newInstance();
...
//                    row17.createCell(1).setCellValue("");
                ...
                runOnUiThread(() -> {
                    ...
                    if (isFileError) {
                        isFileError = false;
                        ToastUtils.showToast(MqttSettings107dProActivity.this, "Export error!");
                        return;
                    }
                    ToastUtils.showToast(MqttSettings107dProActivity.this, "Export success!");
                    Utils.sendEmail(MqttSettings107dProActivity.this, "", "", "Settings for Device", "Choose Email Client", expertFile);
                    ...
                    new Thread(() -> {
                        try {
                            ...
                            if (rows < 18 || columns < 3) {
                                runOnUiThread(() -> {
                                    dismissLoadingProgressDialog();
                                    ToastUtils.showToast(MqttSettings107dProActivity.this, "Please select the correct file!");
                                });
                                return;
                            }
                            ...
                            runOnUiThread(() -> {
                                dismissLoadingProgressDialog();
                                if (isFileError) {
                                    ToastUtils.showToast(MqttSettings107dProActivity.this, "Import failed!");
                                    return;
                                }
                                ToastUtils.showToast(MqttSettings107dProActivity.this, "Import success!");
                                initData();
                            });
                        } catch (Exception e) {
                            e.printStackTrace();
