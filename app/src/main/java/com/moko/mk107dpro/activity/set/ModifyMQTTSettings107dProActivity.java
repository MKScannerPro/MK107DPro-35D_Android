...
import com.moko.mk107dpro.fragment.SSLDeviceUrl107dProFragment;
...
public class ModifyMQTTSettings107dProActivity extends BaseActivity<ActivityMqttDeviceModify107dproBinding> implements RadioGroup.OnCheckedChangeListener {
    ...
    private GeneralDevice107dProFragment generalFragment;
    private UserDevice107dProFragment userFragment;
    private SSLDeviceUrl107dProFragment sslFragment;
    private LWT107dProFragment lwtFragment;
    ...
    protected void onCreate() {
        ...
        expertFilePath = Main107dProActivity.PATH_LOGCAT + File.separator + "export" + File.separator + "Settings for Device.xlsx";
    ...
    @Override
    protected ActivityMqttDeviceModify107dproBinding getViewBinding() {
        return ActivityMqttDeviceModify107dproBinding.inflate(getLayoutInflater());
    }

    private void createFragment() {
        fragments = new ArrayList<>();
        generalFragment = GeneralDevice107dProFragment.newInstance();
        userFragment = UserDevice107dProFragment.newInstance();
        sslFragment = SSLDeviceUrl107dProFragment.newInstance();
        lwtFragment = LWT107dProFragment.newInstance();
...
//                    row17.createCell(1).setCellValue("");
                ...
                runOnUiThread(() -> {
                    ...
                    if (isFileError) {
                        isFileError = false;
                        ToastUtils.showToast(ModifyMQTTSettings107dProActivity.this, "Export error!");
                        return;
                    }
                    ToastUtils.showToast(ModifyMQTTSettings107dProActivity.this, "Export success!");
                    Utils.sendEmail(ModifyMQTTSettings107dProActivity.this, "", "", "Settings for Device", "Choose Email Client", expertFile);
                    ...
                    new Thread(() -> {
                        try {
                            ...
                            if (rows < 18 || columns < 3) {
                                runOnUiThread(() -> {
                                    dismissLoadingProgressDialog();
                                    ToastUtils.showToast(ModifyMQTTSettings107dProActivity.this, "Please select the correct file!");
                                });
                                return;
                            }
                            ...
                            runOnUiThread(() -> {
                                dismissLoadingProgressDialog();
                                if (isFileError) {
                                    ToastUtils.showToast(ModifyMQTTSettings107dProActivity.this, "Import failed!");
                                    return;
                                }
                                ToastUtils.showToast(ModifyMQTTSettings107dProActivity.this, "Import success!");
                                initData();
                            });
                        } catch (Exception e) {
                            e.printStackTrace();
